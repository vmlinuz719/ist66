...
void ch7310_detach(acr7k_subch_t *subch) {
    ...
    fprintf(stderr, "MSC: 7310/%04o:%02o detached\n",
...
void ch7310_sense(
    ...
    ch7310_device_t *device = subch->device;
    fprintf(stderr, "MSC: 7310/%04o:%02o SENSE(%02o, %09o)\n",
        device->ch_id, device->sch_id,
...
void ch7310_control(
    ...
    ch7310_device_t *device = subch->device;
    fprintf(stderr, "MSC: 7310/%04o:%02o CONTROL(%02o, %09o)\n",
        device->ch_id, device->sch_id,
        (unsigned int) opcode, (unsigned int) addr);
}
...
void ch7310_start_transact(
    ...
    fprintf(stderr, "MSC: 7310/%04o:%02o Start Transaction %s(%02o)\n",
        device->ch_id, device->sch_id,
...
void ch7310_end_transact(
    ...
    fprintf(stderr, "MSC: 7310/%04o:%02o End Transaction %s(%02o)\n",
        device->ch_id, device->sch_id,
...
void ch7310_transfer(
    ...
    fprintf(stderr, "MSC: 7310/%04o:%02o     %s(%02o) %09o[%03o]\n",
        device->ch_id, device->sch_id,
        device->write ? "WRITE" : "READ", (unsigned int) device->tx_type,
...
void init_7310(acr7k_cu_t *cpu, int id, int sc_id) {
    ...
    if (sc_id < 0 || sc_id > 15) {
        fprintf(stderr, "MSC: 7310/%04o invalid subchannel %02o\n", id, sc_id);
        return;
    }
    
    else if (ctx->subchannel[sc_id].attached) {
        fprintf(stderr, "MSC: 7310/%04o:%02o already attached\n", id, sc_id);
        return;
    }
    ...
    fprintf(stderr, "MSC: 7310/%04o:%02o (no configurable options)\n",
        device->ch_id, device->sch_id);
