...
/*
 * compute() and its helpers live here rather than in alu.c so that they
 * inline into their call sites. Every caller in cpu.c passes literal
 * constants for op/ci/cond/rc/mk/rt, so inlining folds away the whole
 * rotate/mask/skip pipeline for the common cases; out of line it was also
 * spilling three of its nine arguments to the stack on every call.
 */
